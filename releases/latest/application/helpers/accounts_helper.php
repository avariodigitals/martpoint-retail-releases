...
function insert_account_transaction($data=array()){
	...
	// Guard: if ac_transactions table doesn't exist, skip accounting gracefully
	if(!$CI->db->table_exists('ac_transactions')){
		log_message('error', 'insert_account_transaction: ac_transactions table missing — accounting skipped');
		return true; // Return true so the sale doesn't fail just because accounting is missing
	}
...
function get_cash_account_id(){
	...
	// Guard: if ac_accounts table doesn't exist, return null instead of crashing
	if(!$CI->db->table_exists('ac_accounts')){
		log_message('error', 'get_cash_account_id: ac_accounts table missing — accounting skipped');
		return null;
	}
...
function get_account_balance($account_id){
	$CI =& get_instance();
	if(!$CI->db->table_exists('ac_transactions')) return 0;
...
function update_account_balance($account_id){
	$CI =& get_instance();
	if(!$CI->db->table_exists('ac_accounts')) return true;
...
/**
 * Find the cash account that the current cashier's open shift should use.
 * Falls back to the store's default cash account if no open shift exists.
 * Returns the account ID or null if nothing found.
 */
function get_current_cash_account_id(){
	$CI =& get_instance();
	$store_id = get_current_store_id();
	$user_id  = get_current_user_id();
	$username = $CI->session->userdata('inv_username');
	if(empty($store_id) || empty($user_id)) return null;

	// 1. Is there an open shift for this cashier with a real account?
	if($CI->db->table_exists('db_cashier_shifts')){
		$shift = $CI->db->where('store_id', $store_id)
			->where('cashier_user_id', $user_id)
			->where('status', 'open')
			->order_by('id', 'desc')
			->get('db_cashier_shifts')->row();
		if($shift && !empty($shift->cash_account_id)){
			return (int)$shift->cash_account_id;
		}
		if($shift && !empty($shift->till_id) && $CI->db->table_exists('db_tills')){
			$till = $CI->db->select('account_id')->where('id', $shift->till_id)->where('status', 1)->get('db_tills')->row();
			if($till && !empty($till->account_id)){
				return (int)$till->account_id;
			}
		}
	}

	// 2. Does this cashier have a personal default till?
	if($CI->db->table_exists('db_tills')){
		$till = $CI->db->where('store_id', $store_id)
			->where('(cashier_user_id = '.(int)$user_id.' OR is_default = 1)', NULL, FALSE)
			->where('status', 1)
			->order_by('cashier_user_id', 'desc')
	...
	// 3. Fall back to the store's default cash account
	return get_cash_account_id();
}

/**
 * Ensure a till account exists in ac_accounts for the given till.
 * If account_id is missing, create one and return the ID.
 */
function ensure_till_account($till_name, $store_id, $cashier_user_id = null){
	$CI =& get_instance();
	// Prefer an existing till account linked to this name / cashier
	$q = $CI->db->query("SELECT a.id FROM ac_accounts a
		INNER JOIN db_tills t ON t.account_id = a.id
		WHERE a.store_id = ? AND t.till_name = ? AND a.status = 1 LIMIT 1", array($store_id, $till_name));
	if($q->num_rows() > 0){
		return (int)$q->row()->id;
	}

	// Otherwise create a fresh cash account
	$max_sort = $CI->db->query("SELECT COALESCE(MAX(sort_code),0)+1 as next_sort FROM ac_accounts WHERE store_id = ?", array($store_id))->row()->next_sort;
	$insert = array(
		'count_id'      => $max_sort,
		'store_id'      => $store_id,
		...
		'account_code'  => 'TILL',
		'parent_id'     => 0,
		'account_name'  => 'Till — '.trim($till_name),
		'note'          => 'Cash-in-hand account for till: '.trim($till_name),
		'created_date'  => date('Y-m-d'),
		'created_time'  => date('H:i:s'),
		'created_by'    => $CI->session->userdata('inv_username') ?: 'System',
		'system_ip'     => $CI->input->ip_address(),
