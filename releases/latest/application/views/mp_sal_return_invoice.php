...
<style type="text/css">
  ...
  .mp-status-badge.warranty{background:rgba(217,119,6,.1);color:#b45309;}
...
$status_class = ($return_status=='Cancel') ? 'cancel' : (($return_status=='Warranty') ? 'warranty' : 'return');
?>

<div class="mp-page-head">
  <div>
    <h2><?= $this->lang->line('sales_return_invoice'); ?> #<?php echo $return_code; ?></h2>
    <div class="mp-page-sub"><?= show_date($return_date); ?> &middot; <?= $customer_name; ?></div>
  </div>
  <div class="mp-quick-actions" style="gap:8px;">
    <a href="<?= base_url('sales_return'); ?>" class="mp-qa-btn"><i class="fa fa-arrow-left"></i> Back</a>
    <?php if($CI->permissions('sales_return_edit')) { ?>
    <a href="<?= base_url(($pos==1 ? 'pos/edit/' : 'sales_return/edit/').$return_id); ?>" class="mp-qa-btn blue"><i class="fa fa-edit"></i> Edit</a>
    <?php } ?>
  </div>
</div>
    ...
    <?php } else { ?>
      <div class="alert alert-success text-left" style="margin:0;">
        ...
        <strong>
          ...
          if(!empty($sales_id)){
            echo "<i class='fa fa-fw fa-hand-o-right'></i>Return Against Sales Entry [Sales Code is ".$sales_code.'].';
          } else {
...
<div class="mp-card-form">
  <div class="mp-invoice-head">
    ...
    <div style="flex:1;min-width:240px;">
      <div class="mp-party-title"><?= $this->lang->line('customer_details'); ?></div>
      <div class="mp-party-name"><?= $customer_name; ?></div>
      <?php if(!empty($customer_address) || !empty($customer_country) || !empty($customer_state) || !empty($customer_city) || !empty($customer_postcode)){ ?>
      <div class="mp-party-line"><?php
        echo $customer_address;
        echo $customer_country;
        if(!empty($customer_state)) echo ",".$customer_state;
        if(!empty($customer_city)) echo ",".$customer_city;
        if(!empty($customer_postcode)) echo "-".$customer_postcode;
      ?></div>
      <?php } ?>
      <?php if(!empty(trim($customer_mobile))){ ?><div class="mp-party-line"><?= $this->lang->line('mobile'); ?>: <?= $customer_mobile; ?></div><?php } ?>
      <?php if(!empty(trim($customer_phone))){ ?><div class="mp-party-line"><?= $this->lang->line('phone'); ?>: <?= $customer_phone; ?></div><?php } ?>
      <?php if(!empty(trim($customer_email))){ ?><div class="mp-party-line"><?= $this->lang->line('email'); ?>: <?= $customer_email; ?></div><?php } ?>
      <?php if(!empty(trim($customer_gst_no)) && gst_number()){ ?><div class="mp-party-line"><?= $this->lang->line('gst_number'); ?>: <?= $customer_gst_no; ?></div><?php } ?>
      <?php if(!empty(trim($customer_tax_number))){ ?><div class="mp-party-line"><?= $this->lang->line('tax_number'); ?>: <?= $customer_tax_number; ?></div><?php } ?>
    </div>
    <div class="mp-invoice-meta">
      <div class="mp-invoice-code">#<?= $return_code; ?></div>
      <div class="mp-invoice-date"><?= $this->lang->line('date'); ?>: <?= show_date($return_date)." ".$created_time; ?></div>
      <?php if(!empty($reference_no)){ ?><div class="mp-invoice-date"><?= $this->lang->line('reference_no'); ?>: <?= $reference_no; ?></div><?php } ?>
      <?php if($sales_code){ ?><div class="mp-invoice-date"><?= $this->lang->line('return_against_sales'); ?>: #<?= $sales_code; ?></div><?php } ?>
    ...
    <table class="mp-invoice-table">
      ...
      <tbody>
        ...
        foreach ($q2->result() as $res2) {
            ...
            echo "<td class='num'>".$CI->currency(($res2->price_per_unit * $res2->return_qty))."</td>";
            echo "<td>".store_number_format($res2->tax)."% ".$res2->tax_name." [".$str."]</td>";
      ...
      <tfoot>
        <tr>
          <td colspan="2" class="ctr"><?= $this->lang->line('total'); ?></td>
          <td class="ctr"><?=format_qty($tot_qty);?></td>
          <td class="num"><?= $CI->currency($tot_sales_price);?></td>
          <td>-</td>
...
<div class="mp-card-form" style="margin-top:16px;">
  <div class="mp-card-body">
    <div class="mp-row r-equal" style="align-items:flex-start;">
      ...
      <!-- Left: Coupon, Discount, Notes + Payments -->
      <div>
        <div style="margin-bottom:20px;">
          <?php if(!empty($coupon_code)){ ?>
          <div class="mp-note-row">
            <span class="mp-note-label"><?= $this->lang->line('discountCouponCode'); ?></span>
            <span class="mp-note-value">: <?= getTruncatedCCNumber($coupon_code)?></span>
          </div>
          <?php } ?>
          <?php if(!empty($discount_to_all_input)){ ?>
          <div class="mp-note-row">
            <span class="mp-note-label"><?= $this->lang->line('discount_on_all'); ?></span>
            <span class="mp-note-value">: <?=store_number_format($discount_to_all_input); ?> (<?= $discount_to_all_type ?>)</span>
          </div>
          <?php } ?>
        ...
        <table class="mp-payments-table">
          ...
          <tbody>
            <?php
              if(isset($return_id)){
                $q3p = $this->db->query("select * from db_salespaymentsreturn where return_id=$return_id");
                if($q3p->num_rows()>0){
                  $pi=1;
                  $total_paid = 0;
                  foreach ($q3p->result() as $res3p) {
                    ...
                    echo "<td>".payment_mode_label($res3p->payment_type)."</td>";
      ...
      <div>
        <table class="mp-invoice-totals">
          ...
          <tr>
            <td><?= $this->lang->line('couponDiscount'); ?> <?= ($coupon_type=='Percentage') ? $coupon_value .'%' : (($coupon_value>0) ? $coupon_value.' [Fixed]' : '');?></td>
            <td id="coupon_discount_amt" name="coupon_discount_amt"><?=$CI->currency($coupon_amt);?></td>
          </tr>
          <tr>
  ...
  <div class="mp-card-body" style="padding:16px 20px;">
    <div class="mp-form-actions" style="justify-content:space-between;flex-wrap:wrap;gap:12px;">
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <?php if($CI->permissions('sales_return_edit')) { ?>
        <a href="<?= base_url(($pos==1 ? 'pos/edit/' : 'sales_return/edit/').$return_id); ?>" class="mp-btn-primary"><i class="fa fa-edit"></i> Edit</a>
        <?php } ?>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <a href="<?= base_url('sales_return/print_invoice/'.$return_id); ?>" target="_blank" class="mp-btn-secondary"><i class="fa fa-print"></i> Print</a>
        <a href="<?= base_url('sales_return/pdf/'.$return_id); ?>" target="_blank" class="mp-btn-secondary"><i class="fa fa-file-pdf-o"></i> PDF</a>
      </div>
    </div>
  </div>
</div>
