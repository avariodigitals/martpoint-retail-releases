...
<div class="mp-section">
  <div class="mp-page-head">
    <div>
      <h2><?= $page_title; ?></h2>
      <div class="mp-page-sub"><?= htmlspecialchars($store_name); ?> &mdash; Import attributes from a CSV file</div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
      <a href="<?php echo $base_url; ?>attributes" class="mp-qa-btn" style="background:var(--mp-bg);color:var(--mp-ink);border:1px solid var(--mp-border);">
  ...
  <div class="mp-card-form box">
    ...
    <form id="import-form" enctype="multipart/form-data" method="POST">
      ...
      <div class="mp-card-body">
        <div class="mp-form-grid">
          <div class="mp-form-group full">
            <label><?= $this->lang->line('import_attributes'); ?> <span class="text-danger">*</span></label>
            ...
            <p class="mp-form-hint">File must be in CSV format. <a href="<?= base_url('import/download/attributes'); ?>" style="color:var(--mp-primary);font-weight:600;">Download example format</a></p>
      ...
      <div class="mp-form-actions">
        ...
        <a href="<?php echo $base_url; ?>attributes" class="mp-qa-btn" style="background:var(--mp-bg);color:var(--mp-ink);border:1px solid var(--mp-border);">Cancel</a>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="mp-card-form">
    ...
    <div class="mp-card-body" style="padding:0;">
      <table class="mp-tbl" id="report-data">
        ...
        <tbody id="tbodyid">
          <?php $i=1; ?>
          <tr><td><?= $i++; ?></td><td><?= $this->lang->line('attribute_type'); ?></td><td><span class="mp-pill ok"><?= $this->lang->line('required'); ?></span></td><td>e.g. size, colour, length, material</td></tr>
          <tr><td><?= $i++; ?></td><td><?= $this->lang->line('attribute_value'); ?></td><td><span class="mp-pill ok"><?= $this->lang->line('required'); ?></span></td><td>Existing type/value pairs are skipped</td></tr>
          <tr><td><?= $i++; ?></td><td><?= $this->lang->line('sort_order'); ?></td><td><span class="mp-pill default"><?= $this->lang->line('optional'); ?></span></td><td>Number, defaults to 0</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
<script type="text/javascript">
  $("#import").on("click", function(e) {
    ...
    function doImportAttributes() {
      ...
      $.ajax({
        type: 'POST',
        url: base_url + 'import/import_attributes_csv',
        data: data,
        cache: false,
        contentType: false,
        processData: false,
        success: function(result) {
          if (result == "success") {
            toastr["success"]("Attributes imported successfully.");
            window.location = base_url + "attributes";
    ...
    if (typeof swal === 'undefined') {
      ...
      swal({
        title: "Are you sure?",
        text: "This will import attributes from the selected CSV file.",
        icon: "warning",
        buttons: true,
        dangerMode: true
      }).then(function(willImport) {
        if (willImport) doImportAttributes();
      });
    }
  });
</script>
