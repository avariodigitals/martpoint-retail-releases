...
<div class="mp-section">
  <div class="mp-page-head">
    <div>
      <h2><?= $page_title; ?></h2>
      <div class="mp-page-sub"><?= htmlspecialchars($store_name); ?> &mdash; Import brands from a CSV file</div>
    </div>
      ...
      <div class="mp-card-body">
        <div class="mp-form-grid">
          <div class="mp-form-group full">
            <label><?= $this->lang->line('import_brands'); ?> <span class="text-danger">*</span></label>
            ...
            <p class="mp-form-hint">File must be in CSV format. <a href="<?= base_url('import/download/brands'); ?>" style="color:var(--mp-primary);font-weight:600;">Download example format</a></p>
  ...
  <div class="mp-card-form">
    ...
    <div class="mp-card-body" style="padding:0;">
      <table class="mp-tbl" id="report-data">
        ...
        <tbody id="tbodyid">
          <?php $i=1; ?>
          <tr><td><?= $i++; ?></td><td><?= $this->lang->line('brand_name'); ?></td><td><span class="mp-pill ok"><?= $this->lang->line('required'); ?></span></td><td>Existing brand names are skipped</td></tr>
...
<script type="text/javascript">
  $("#import").on("click", function(e) {
    ...
    function doImportBrands() {
      ...
      $.ajax({
        type: 'POST',
        url: base_url + 'import/import_brands_csv',
        data: data,
        cache: false,
        contentType: false,
        processData: false,
        success: function(result) {
          if (result == "success") {
            toastr["success"]("Brands imported successfully.");
    ...
    if (typeof swal === 'undefined') {
      ...
      swal({
        title: "Are you sure?",
        text: "This will import brands from the selected CSV file.",
        icon: "warning",
        buttons: true,
        dangerMode: true
      }).then(function(willImport) {
        if (willImport) doImportBrands();
      });
    }
  });
</script>
