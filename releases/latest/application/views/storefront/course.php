...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .cra-title { font-size:28px; font-weight:900; margin-bottom:8px; }
    .cra-subtitle { font-size:15px; color:var(--gray); line-height:1.6; margin-bottom:20px; }
    .cra-progress { height:10px; background:var(--light-gray); border-radius:999px; overflow:hidden; margin-bottom:24px; }
    .cra-progress > div { height:100%; background:var(--primary); border-radius:999px; }
    .cra-module-title { font-size:12px; font-weight:800; color:var(--gray); text-transform:uppercase; letter-spacing:0.05em; margin:24px 0 12px; }
    .cra-lesson { padding:14px 0; border-bottom:1px solid var(--light-gray); }
    .cra-lesson:last-child { border-bottom:none; }
    .cra-lesson-title { font-size:14px; font-weight:700; }
    .cra-lesson-desc { font-size:13px; color:var(--gray); margin-top:4px; line-height:1.6; }
    .cra-lesson-done { color:var(--success); font-size:12px; font-weight:700; }
    .cra-lesson-link { color:var(--primary); font-weight:600; }
    .cra-content { font-size:14px; line-height:1.7; color:var(--dark); }
    .cra-content p { margin-bottom:12px; }
...
<div class="cra-topbar">
  <div class="cra-topbar-inner">
    <a href="<?= base_url('store/' . ($settings->store_slug ?? '') . '/my_courses'); ?>" class="cra-back">
...
<div class="cra-layout">
  ...
  <main class="cra-main">
    <div class="cra-card">
      <div class="cra-title"><?= htmlspecialchars($course->title); ?></div>
      <div class="cra-subtitle"><?= (int) $progress; ?>% complete</div>
      <div class="cra-progress"><div style="width:<?= (int) $progress; ?>%"></div></div>

      <?php if(!empty($course->description)): ?>
      <div style="margin-bottom:24px;">
        <div class="cra-card-title" style="margin-bottom:10px;">About this course</div>
        <div class="cra-content"><?= nl2br(htmlspecialchars($course->description)); ?></div>
      </div>
      <?php endif; ?>
      ...
      <?php if(!empty($modules) || !empty($lessons)): ?>
        <?php
        $lessonsByModule = ['' => []];
        foreach($modules as $m) $lessonsByModule[$m->id] = [];
        foreach($lessons as $l) {
            $key = $l->module_id ?? '';
            $lessonsByModule[$key][] = $l;
        }
        ?>
        <?php foreach($modules as $module): ?>
        <div class="cra-module-title"><?= htmlspecialchars($module->title); ?></div>
        <div class="cra-card" style="padding:20px;">
          <?php foreach($lessonsByModule[$module->id] ?? [] as $lesson): ?>
          <div class="cra-lesson">
            <div class="cra-lesson-title"><?= htmlspecialchars($lesson->title); ?> <?= in_array($lesson->id, $completed_ids) ? '<span class="cra-lesson-done">&#10003; Done</span>' : ''; ?></div>
            <?php if(!empty($lesson->content) || !empty($lesson->video_url) || !empty($lesson->video_file)): ?>
            <div class="cra-lesson-desc">
              <?php if(!empty($lesson->video_url) || !empty($lesson->video_file)): ?>
                <a href="<?= !empty($lesson->video_url) ? htmlspecialchars($lesson->video_url) : base_url($lesson->video_file); ?>" target="_blank" class="cra-lesson-link">Watch video</a>
              <?php endif; ?>
              <?php if(!empty($lesson->content)): ?>
                <?= nl2br(htmlspecialchars(strip_tags($lesson->content))); ?>
              <?php endif; ?>
            </div>
            <?php endif; ?>
          </div>
        ...
        <?php endforeach; ?>
        <?php if(!empty($lessonsByModule[''])): ?>
        <div class="cra-card" style="padding:20px;">
          <?php foreach($lessonsByModule[''] as $lesson): ?>
          <div class="cra-lesson">
            <div class="cra-lesson-title"><?= htmlspecialchars($lesson->title); ?> <?= in_array($lesson->id, $completed_ids) ? '<span class="cra-lesson-done">&#10003; Done</span>' : ''; ?></div>
            <?php if(!empty($lesson->content) || !empty($lesson->video_url) || !empty($lesson->video_file)): ?>
            <div class="cra-lesson-desc">
        ...
        <?php endif; ?>
      <?php else: ?>
        <div class="cra-empty">No lessons published yet.</div>
      <?php endif; ?>
    </div>
