...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .cra-table th, .cra-table td { padding:14px 0; text-align:left; font-size:14px; border-bottom:1px solid var(--light-gray); }
    .cra-table th { color:var(--gray); font-size:12px; text-transform:uppercase; letter-spacing:.5px; font-weight:700; }
    .cra-table tr:last-child td { border-bottom:none; }
    .cra-code { font-family:monospace; font-weight:800; color:var(--primary); }
    ...
    .cra-badge-processing { background:#DBEAFE; color:#1E40AF; }
    ...
    @media(max-width:900px){
      ...
      .cra-table { font-size:12px; }
      .cra-table th, .cra-table td { padding:10px 0; }
    }
  </style>
</head>
<body>
...
<div class="cra-layout">
  <aside class="cra-sidebar">
    ...
    <nav class="cra-menu">
      ...
      <a href="<?= base_url('store/' . ($settings->store_slug ?? '') . '/account/orders'); ?>" class="active"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg> Orders</a>
  ...
  <main class="cra-main">
    <div class="cra-card">
      <div class="cra-card-header">
        ...
        <a href="<?= base_url('store/' . ($settings->store_slug ?? '') . '/track'); ?>" class="cra-card-link">Track order</a>
      </div>
      ...
      <div style="overflow-x:auto;">
        <table class="cra-table">
          ...
          <tbody>
            ...
            <tr>
              <td class="cra-code"><?= htmlspecialchars($o->order_code); ?></td>
              <td><?= date('M j, Y', strtotime($o->created_at ?? 'now')); ?></td>
              <td><span class="cra-badge <?= in_array($o->order_status, ['completed','paid']) ? 'cra-badge-active' : (in_array($o->order_status, ['pending','processing']) ? 'cra-badge-processing' : 'cra-badge-pending'); ?>"><?= ucfirst($o->order_status); ?></span></td>
              <td><?= htmlspecialchars($settings->currency ?? '₦'); ?><?= number_format($o->grand_total, 2); ?></td>
              <td><a href="<?= base_url('store/' . ($settings->store_slug ?? '') . '/order_received/' . urlencode($o->order_code)); ?>" class="cra-card-link">View</a></td>
            </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
      ...
      <?php else: ?>
        <div class="cra-empty">You haven't placed any orders yet.</div>
      <?php endif; ?>
    </div>
