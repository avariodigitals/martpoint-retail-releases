...
<h3><i class="fa fa-credit-card"></i> Store Credit Card Preview</h3>
<button class="btn btn-primary" onclick="window.print()"><i class="fa fa-print"></i> Print Card</button>
<button class="btn btn-success" onclick="downloadCard('storeCreditPrint','store-credit-<?=htmlspecialchars($credit->credit_code ?? 'export');?>.png')"><i class="fa fa-download"></i> Download PNG</button>
<a href="<?=base_url();?>store_credit" class="btn btn-default"><i class="fa fa-arrow-left"></i> Back</a>
</div>

<div class="card-container" id="storeCreditPrint">
<div class="card-brand"><?=htmlspecialchars($brand_name ?? 'MartPoint');?></div>
<div class="card-barcode">
<img src="https://bwipjs-api.metafloor.com/?bcid=code128&text=<?=urlencode($credit->credit_code ?? 'SC0000');?>&scale=3&height=8" alt="barcode">
</div>
<div class="card-watermark"><?=htmlspecialchars($brand_name ?? 'MartPoint');?></div>
<div class="card-body">
<div class="card-label">Store Credit Value</div>
<div class="card-value"><?=store_number_format($credit->amount ?? 0);?></div>
<div class="card-number"><?=htmlspecialchars($credit->credit_code ?? '');?></div>
<div class="card-name"><?=htmlspecialchars($customer->customer_name ?? 'Store Credit');?></div>
<div class="card-type-label">Store Credit</div>
</div>
<div class="card-footer">
<div class="card-signature"><?=htmlspecialchars($brand_name ?? 'MartPoint');?></div>
<div class="card-expiry"><?=($credit->expiry_date ? 'Expires: ' . show_date($credit->expiry_date) : 'No Expiry');?></div>
<div style="font-size:9px;color:#8b7355;">ID: <?=str_pad($credit->id ?? 0, 4, '0', STR_PAD_LEFT);?></div>
</div>
</div>
