...
<style type="text/css">
  ...
  .mp-status-badge.draft{background:rgba(120,113,108,.1);color:var(--mp-muted);}
  .mp-status-badge.ordered{background:rgba(0,87,255,.1);color:var(--mp-primary);}
  .mp-status-badge.partial{background:rgba(245,158,11,.1);color:var(--mp-warning);}
  .mp-status-badge.received{background:rgba(5,150,105,.1);color:var(--mp-success);}
...
$status_class = 'draft';
if($purchase_status=='Ordered') $status_class='ordered';
elseif($purchase_status=='Partially Received') $status_class='partial';
elseif($purchase_status=='Received') $status_class='received';
?>

<div class="mp-page-head">
  <div>
    <h2><?= $this->lang->line('purchase_invoice'); ?> #<?php echo $purchase_code; ?></h2>
    <div class="mp-page-sub"><?= show_date($purchase_date); ?> &middot; <?= $supplier_name; ?></div>
  </div>
  <div class="mp-quick-actions" style="gap:8px;">
    <a href="<?= base_url('purchase'); ?>" class="mp-qa-btn"><i class="fa fa-arrow-left"></i> Back</a>
    <?php if($CI->permissions('purchase_edit')) { ?>
    <a href="<?= base_url('purchase/update/'.$purchase_id); ?>" class="mp-qa-btn blue"><i class="fa fa-edit"></i> Edit</a>
    <?php } ?>
  </div>
</div>
...
<div class="mp-card-form">
  <div class="mp-invoice-head">
    ...
    <div class="mp-invoice-meta">
      <div class="mp-invoice-code">#<?= $purchase_code; ?></div>
      <div class="mp-invoice-date"><?= $this->lang->line('purchase_date'); ?>: <?= show_date($purchase_date); ?></div>
      <?php if(!empty($reference_no)){ ?><div class="mp-invoice-date"><?= $this->lang->line('reference_no'); ?>: <?= $reference_no; ?></div><?php } ?>
      <div class="mp-status-badge <?= $status_class; ?>"><?= $purchase_status; ?></div>
    </div>
  </div>
</div>
    ...
    <table class="mp-invoice-table">
      ...
      <tbody>
        ...
        foreach ($q2->result() as $res2) {
            ...
            echo "<td class='ctr'>".format_qty($res2->purchase_qty)."</td>";
...
<div class="mp-card-form" style="margin-top:16px;">
  <div class="mp-card-body">
    <div class="mp-row r-equal" style="align-items:flex-start;">
      ...
      <!-- Left: Notes + Payments -->
      <div>
        <?php if(!empty($discount_to_all_input) || !empty($purchase_note)){ ?>
        <div style="margin-bottom:20px;">
          ...
          <?php } ?>
          <?php if(!empty($purchase_note)){ ?>
          <div class="mp-note-row">
            <span class="mp-note-label"><?= $this->lang->line('note'); ?></span>
            <span class="mp-note-value">: <?= nl2br(htmlspecialchars($purchase_note));?></span>
          </div>
          <?php } ?>
        </div>
        <?php } ?>
          ...
          <tbody>
            <?php
              if(isset($purchase_id)){
                $q3p = $this->db->query("select * from db_purchasepayments where purchase_id=$purchase_id");
                if($q3p->num_rows()>0){
                  $pi=1;
                  $total_paid = 0;
  ...
  <div class="mp-card-body" style="padding:16px 20px;">
    <div class="mp-form-actions" style="justify-content:space-between;flex-wrap:wrap;gap:12px;">
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ...
        <a href="<?= base_url('purchase/update/'.$purchase_id); ?>" class="mp-btn-primary"><i class="fa fa-edit"></i> Edit</a>
        <?php } ?>
        <a href='<?= base_url('items/labels/'.$purchase_id);?>' class="mp-btn-secondary"><i class="fa fa-barcode"></i> Barcode</a>
        <?php if($CI->permissions('purchase_return_add')) { ?>
        <a href="<?= base_url('purchase_return/add/'.$purchase_id); ?>" class="mp-btn-secondary" style="color:var(--mp-danger);border-color:rgba(220,38,38,.2);"><i class="fa fa-undo"></i> Purchase Return</a>
        <?php } ?>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <a href="<?= base_url('purchase/print_invoice/'.$purchase_id); ?>" target="_blank" class="mp-btn-secondary"><i class="fa fa-print"></i> Print</a>
        <a href="<?= base_url('purchase/pdf/'.$purchase_id); ?>" target="_blank" class="mp-btn-secondary"><i class="fa fa-file-pdf-o"></i> PDF</a>
        <?php $wa_share = get_whatsapp_share_url('purchase',$purchase_id); ?>
        <a href="<?= $wa_share['url']; ?>" target="_blank" class="mp-btn-secondary" style="background-color:#25D366;color:#fff;border-color:#25D366;"><i class="fa fa-whatsapp"></i> Share WhatsApp</a>
      </div>
    </div>
  </div>
</div>
