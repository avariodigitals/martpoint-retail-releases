   ...
   <body class="hold-transition skin-blue sidebar-mini">
      ...
      <div class="wrapper">
         ...
         <?php 
            ...
            $total_invoice = $this->db->select("COUNT(*) as total")->from("db_purchase")->where("store_id", get_current_store_id())->get()->row()->total;
            ...
            $pur_return_total = $this->db->select("COALESCE(SUM(grand_total),0) AS pur_total")->from("db_purchasereturn")->where("store_id", get_current_store_id())->where("purchase_id is not null")->get()->row()->pur_total;
             //total purchase due
            if (!is_admin()) {
              if ($this->session->userdata('role_id') != '2') {
                $this->db->where("upper(created_by)", strtoupper($this->session->userdata('inv_username')));
            ...
            $purchase_due_total = $this->db->select("COALESCE(SUM(purchase_due),0) AS purchase_due")->from("db_suppliers")->where("store_id", get_current_store_id())->get()->row()->purchase_due;
            ...
            $tot_paid_amt=$this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount")->from("db_purchase")->where("store_id",get_current_store_id())->get()->row()->paid_amount;
  ...
  <!-- ********** ALERT MESSAGE START******* -->
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
          <div class="info-box " >
            ...
            <div class="info-box-content">
              ...
              <span class="info-box-number"><?=$this->lang->line('total_paid_amount');?></span>
            </div>
            <!-- /.info-box-content -->
          </div>
          <!-- /.info-box -->
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
          <div class="info-box " >
            ...
            <div class="info-box-content">
              <span class="info-box-text"><?=$CI->currency(kmb($purchase_due_total));?></span>
                        ...
                        <div class="box-header with-border">
                          ...
                          <div class="col-xs-8 input-group">
                           ...
                           <div class="box-tools">
                              ...
                              <i class="fa fa-plus"></i> <?= $this->lang->line('create_purchase'); ?></a>
                           </div>
                           <?php } ?>
                        </div>
                        <!-- /.box-header -->
      ...
      <script type="text/javascript">
        function show_change_status_modal(purchase_id){
          ...
          $.post(base_url+'purchase/show_change_status_modal', {purchase_id: purchase_id}, function(result) {
            ...
            // Initialize the batch fields after modal loads
