...
<div class="mp-page-head">
  <div>
    ...
    <div class="mp-page-sub">Record cash and bank deposits</div>
  </div>
  <?php if($CI->permissions('money_deposit_add')) { ?>
  <a class="mp-qa-btn green" href="<?= base_url('money_deposit/add'); ?>">
    <i class="fa fa-plus"></i> <?= $this->lang->line('new_deposit'); ?>
  </a>
  <?php } ?>
</div>
  ...
  <div class="box-body">
    ...
    <div class="row" style="padding:16px 20px;border-bottom:1px solid var(--mp-border);display:flex;gap:16px;flex-wrap:wrap;align-items:flex-end;">
      <div class="col-md-3 col-sm-6" style="min-width:200px;">
        <label><?= $this->lang->line('deposit_date'); ?></label>
...
function load_datatable(){
   var table = $('#example2').DataTable({
      ...
      "ajax": {
          "url": "<?= base_url('money_deposit/ajax_list'); ?>",
          "type": "POST",
          "data": {
...
$("#deposit_date,#credit_account_id,#debit_account_id,#users").on("change",function(){
