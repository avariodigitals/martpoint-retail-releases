...
$paid_amount = $this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount")
                         ->from("db_purchasereturn")
                         ->where("store_id", get_current_store_id())
                         ->get()->row()->paid_amount;
...
$purchase_due_total = $this->db->select("COALESCE(SUM(purchase_return_due),0) AS purchase_return_due")
                                ->from("db_suppliers")
                                ->where("store_id", get_current_store_id())
                                ->get()->row()->purchase_return_due;
?>

<div class="mp-page-head">
  <div>
    <h2><?=$page_title;?></h2>
    <div class="mp-page-sub">View and manage purchase returns &middot; <?=date('F j, Y');?></div>
  </div>
  <?php if($CI->permissions('purchase_return_add')): ?>
  <a href="<?=base_url('purchase_return/create');?>" class="mp-qa-btn green"><i class="fa fa-plus"></i> New Return</a>
  <?php endif; ?>
</div>

<div class="mp-kpi-grid">
  <div class="mp-kpi-card sales">
    <div class="mp-kpi-icon"><i class="fa fa-undo"></i></div>
    <div class="mp-kpi-label">Total Invoices</div>
    <div class="mp-kpi-value"><?=number_format($total_invoice);?></div>
    <div class="mp-kpi-sub neutral">Return invoices</div>
  </div>
  <div class="mp-kpi-card expense">
    ...
    <div class="mp-kpi-sub neutral">Total returned value</div>
  </div>
  <div class="mp-kpi-card profit">
    ...
    <div class="mp-kpi-sub neutral">Amount refunded</div>
  </div>
  <div class="mp-kpi-card debt">
    ...
    <div class="mp-kpi-sub neutral">Return dues</div>
  </div>
</div>
