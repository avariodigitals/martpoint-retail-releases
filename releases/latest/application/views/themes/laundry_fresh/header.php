...
<div class="mp-header">
  <div class="mp-header-main">
    ...
    <div class="mp-header-search">
      ...
      <input type="text" id="search-input" placeholder="Search laundry services..." onkeydown="if(event.key==='Enter')doSearch()">
    </div>
    <?php endif; ?>
    ...
    <div class="mp-header-actions">
      ...
      <a href="<?= base_url('store/' . $slug . '/services'); ?>" class="laundry-header-cta" aria-label="Schedule a pickup">Schedule a Pickup</a>
    </div>
  </div>
</div>
...
<div class="mp-nav" id="mp-nav">
  <div class="mp-nav-inner">
    ...
    <a href="<?= base_url('store/' . $slug . '/branches'); ?>" class="mp-nav-link">Branches</a>
    <a href="<?= base_url('store/' . $slug . '/track'); ?>" class="mp-nav-link">Track Order</a>
    <?php if(!empty($serviceCats)): foreach(array_slice($serviceCats, 0, 4) as $cat): ?>
    <a href="<?= base_url('store/' . $slug . '/services?category=' . $cat->id); ?>" class="mp-nav-link"><?= htmlspecialchars($cat->category_name); ?></a>
...
<div id="mobile-menu" style="display:none; position:fixed; top:0; left:0; bottom:0; width:280px; background:#fff; z-index:901; padding:24px; box-shadow:4px 0 24px rgba(0,0,0,0.1);">
  ...
  <div style="display:flex; flex-direction:column; gap:16px;">
    ...
    <a href="<?= base_url('store/' . $slug . '/branches'); ?>" style="font-size:16px; font-weight:600; color:var(--mp-dark);" onclick="toggleMobileMenu()">Branches</a>
    <a href="<?= base_url('store/' . $slug . '/track'); ?>" style="font-size:16px; font-weight:600; color:var(--mp-dark);" onclick="toggleMobileMenu()">Track Order</a>
