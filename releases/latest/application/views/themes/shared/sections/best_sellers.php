...
<div class="mp-section">
  <div class="mp-section-title">
    <?= htmlspecialchars(sf_sec_title($section, 'Best Sellers')); ?>
  ...
  <div class="mp-grid">
    ...
    <div class="mp-card" onclick="openProductModal(<?= $p->id; ?>, '<?= htmlspecialchars(addslashes($p->item_name)); ?>', <?= $price; ?>, '<?= $p->item_image; ?>', '<?= htmlspecialchars(addslashes($p->description ?? '')); ?>', <?= $p->stock; ?>, <?= $hasDiscount ? $oldPrice : 0; ?>)">
      <div class="mp-card-img-wrap">
        ...
        <?php endif; ?>
        <span class="mp-card-badge bestseller">Bestseller</span>
        <?php if($badgeLabel): ?>
          <span class="mp-card-badge" style="background:rgba(124,58,237,.12);color:#7C3AED;left:auto;right:14px;"><?= $badgeLabel; ?></span>
        <?php endif; ?>
