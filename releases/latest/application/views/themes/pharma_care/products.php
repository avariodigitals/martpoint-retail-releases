<?php
/**
 * Pharma Care — Products Listing
 * Warm community pharmacy product grid with deep teal and amber accents.
 */
...
<style>
  ...
  .pcr-page { padding:36px 0 72px; }
  .pcr-page-head { margin-bottom:32px; }
  ...
  .pcr-filters { display:flex; gap:12px; align-items:center; margin-bottom:32px; flex-wrap:wrap; }
  .pcr-search-bar { flex:1; max-width:420px; position:relative; }
  .pcr-search-bar input { width:100%; padding:12px 14px 12px 42px; border:1px solid var(--pcr-border); border-radius:8px; font-family:'Inter',sans-serif; font-size:14px; background:var(--pcr-white); outline:none; transition:border-color .2s; color:var(--pcr-dark); }
  .pcr-search-bar input:focus { border-color:var(--pcr-teal); }
  .pcr-search-icon { position:absolute; left:14px; top:50%; transform:translateY(-50%); color:var(--pcr-teal); }
  .pcr-filter-chip { padding:10px 20px; border-radius:8px; border:1px solid var(--pcr-border); background:var(--pcr-white); font-family:'Inter',sans-serif; font-size:13px; font-weight:600; color:var(--pcr-gray); cursor:pointer; transition:all .2s; text-decoration:none; display:inline-block; }
  .pcr-filter-chip.active, .pcr-filter-chip:hover { background:var(--pcr-teal); color:#fff; border-color:var(--pcr-teal); }
  ...
  .pcr-product-card { position:relative; background:var(--pcr-white); border-radius:16px; overflow:hidden; border:1px solid var(--pcr-border); transition:transform .25s, box-shadow .25s; cursor:pointer; }
  ...
  .pcr-pagination { display:flex; justify-content:center; gap:8px; margin-top:44px; }
  .pcr-page-btn { padding:10px 16px; border-radius:8px; border:1px solid var(--pcr-border); background:var(--pcr-white); font-family:'Inter',sans-serif; font-size:14px; font-weight:600; color:var(--pcr-gray); text-decoration:none; transition:all .2s; }
  .pcr-page-btn:hover { border-color:var(--pcr-teal); color:var(--pcr-teal); }
  .pcr-page-btn.active { background:var(--pcr-teal); color:#fff; border-color:var(--pcr-teal); }

  .pcr-empty { text-align:center; padding:88px 20px; color:var(--pcr-gray); }
  .pcr-empty-icon { font-size:48px; margin-bottom:14px; color:var(--pcr-teal); }
  .pcr-empty-title { font-family:'Lora',serif; font-size:22px; font-weight:700; color:var(--pcr-dark); margin-bottom:10px; }
  .pcr-empty-text { font-family:'Inter',sans-serif; margin-bottom:22px; }
  .pcr-empty .pcr-btn-amber { padding:14px 34px; border-radius:8px; background:var(--pcr-amber); color:#fff; font-family:'Inter',sans-serif; font-weight:600; font-size:13px; text-decoration:none; display:inline-block; }
...
<div class="pcr-container">
  ...
  <div class="pcr-page">
    <div class="pcr-page-head">
      <div class="pcr-section-label">Health Essentials</div>
      <h1 class="pcr-section-title"><?= !empty($search) ? 'Search: ' . htmlspecialchars($search) : 'All Products'; ?></h1>
    </div>

    <div class="pcr-filters">
      <div class="pcr-search-bar">
        <span class="pcr-search-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></span>
        <input type="text" placeholder="Search health products..." value="<?= htmlspecialchars($search ?? ''); ?>" onkeydown="if(event.key==='Enter'){const u=new URL(location.href);u.searchParams.set('search',this.value);u.searchParams.delete('page');location.href=u.href;}">
      </div>
      <?php if(!empty($categories)): ?>
      <a href="<?= base_url('store/' . $slug . '/products'); ?>" class="pcr-filter-chip <?= empty($category_id) ? 'active' : ''; ?>">All</a>
      <?php foreach($categories as $cat): ?>
      <a href="<?= base_url('store/' . $slug . '/products?category=' . $cat->id); ?>" class="pcr-filter-chip <?= ($category_id ?? 0) == $cat->id ? 'active' : ''; ?>"><?= htmlspecialchars($cat->category_name); ?></a>
      <?php endforeach; ?>
      <?php endif; ?>
    </div>
    ...
    <div class="pcr-pagination">
      <?php if($page > 1): ?>
        <a class="pcr-page-btn" href="<?= base_url('store/' . $slug . '/products'); ?>?page=<?= $page-1; ?><?= !empty($category_id) ? '&category=' . $category_id : ''; ?><?= !empty($search) ? '&search=' . urlencode($search) : ''; ?>">&laquo;</a>
      <?php endif; ?>
      <?php for($i = max(1, $page-2); $i <= min($total_pages, $page+2); $i++): ?>
        <a class="pcr-page-btn <?= $i == $page ? 'active' : ''; ?>" href="<?= base_url('store/' . $slug . '/products'); ?>?page=<?= $i; ?><?= !empty($category_id) ? '&category=' . $category_id : ''; ?><?= !empty($search) ? '&search=' . urlencode($search) : ''; ?>"><?= $i; ?></a>
      <?php endfor; ?>
      <?php if($page < $total_pages): ?>
        <a class="pcr-page-btn" href="<?= base_url('store/' . $slug . '/products'); ?>?page=<?= $page+1; ?><?= !empty($category_id) ? '&category=' . $category_id : ''; ?><?= !empty($search) ? '&search=' . urlencode($search) : ''; ?>">&raquo;</a>
      <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <?php else: ?>
    <div class="pcr-empty">
      <div class="pcr-empty-icon"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg></div>
      <div class="pcr-empty-title">No health products found</div>
      <p class="pcr-empty-text">Try a different search or browse all products.</p>
      <a href="<?= base_url('store/' . $slug . '/products'); ?>" class="pcr-btn-amber">View All Products</a>
    </div>
    <?php endif; ?>
  </div>
</div>
