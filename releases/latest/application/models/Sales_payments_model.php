...
class Sales_payments_model extends CI_Model {
	...
	public function delete_payment($payment_id){
		...
		$salespayments = $this->db->where('id', $payment_id)->select('sales_id,customer_id')->get('db_salespayments')->row();
		$sales_id = $salespayments->sales_id;
		$customer_id = $salespayments->customer_id;

		$q1 = $this->db->where('id', $payment_id)->delete('db_salespayments');
		if(!$q1){
			return "failed";
		}
	...
	public function show_cheque_payments_modal($payment_id){
		...
		$q1 = $this->db->where('id', $sales_id)->get('db_sales');
		$res1=$q1->row();
		$customer_id = $res1->customer_id;
		$q2 = $this->db->where('id', $customer_id)->get('db_customers');
		$res2=$q2->row();
	    ...
	    if(!empty($customer_country)){
	      $customer_country = $this->db->where('id', $customer_country)->get('db_country')->row()->country;  
	    }
	    if(!empty($customer_state)){
	      $customer_state = $this->db->where('id', $customer_state)->get('db_states')->row()->state;  
	    }
	...
	public function update_cheque_payment(){
		// Explicit extraction (security: never use extract on user input)
		$cheque_status = $this->input->post('cheque_status', TRUE);
		$payment_id = $this->input->post('payment_id', TRUE);
    	$q1 = $this->db->where('id', $payment_id)->update('db_salespayments', ['cheque_status' => $cheque_status]);
    	if(!$q1){
    		return "failed";
    	}
    	return "success";
