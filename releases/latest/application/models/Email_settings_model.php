...
class Email_settings_model extends CI_Model {
	...
	private function ensureTable(){
		if(!$this->db->table_exists('db_email_settings')){
			log_message('error', 'Missing required table: db_email_settings. Run the 4.0.2 migration via login.');
		}
	}
	...
	public function saveSettings($storeId, array $data){
		...
		// Write SMTP settings to the modular notification table instead of db_store
		if(!empty($storeUpdate)){
			if($this->db->table_exists('db_store_notification_settings')){
				$exists = $this->db->where('store_id', $storeId)->get('db_store_notification_settings')->row();
				if($exists){
					$ok = $this->db->where('store_id', $storeId)->update('db_store_notification_settings', $storeUpdate) && $ok;
				} else {
					$storeUpdate['store_id'] = $storeId;
					$ok = $this->db->insert('db_store_notification_settings', $storeUpdate) && $ok;
				}
			} else {
				// Fallback for legacy installations without the modular table
				$ok = $this->db->where('id', $storeId)->update('db_store', $storeUpdate) && $ok;
			}
		}
