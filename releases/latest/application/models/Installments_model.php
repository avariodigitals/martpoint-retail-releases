...
class Installments_model extends CI_Model {
	...
	public function create_plan($data){
		...
		// Use a temp plan code because the real one needs the auto-increment id
		$plan = array(
			'store_id'            => $data['store_id'],
			'sales_id'            => isset($data['sales_id']) ? $data['sales_id'] : null,
			'customer_id'         => $data['customer_id'],
			'plan_code'           => 'TMP-' . time() . '-' . mt_rand(1000, 9999),
		...
		// Update plan_code with the real id
		$plan_code = 'IN' . date('Y') . '/' . str_pad($store_id, 2, '0', STR_PAD_LEFT) . '/' . $plan_id;
		$this->db->where('id', $plan_id)->update('db_installment_plans', array('plan_code' => $plan_code));
