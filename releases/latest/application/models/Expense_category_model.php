...
class Expense_category_model extends CI_Model {
	...
	public function verify_and_save(){
		...
		$this->db->where("upper(category_name)", strtoupper($category));
		...
		if($query->num_rows()>0){
			...
			$q1 = $this->db->insert('db_expense_category', $info);
			if ($q1){
	...
	public function update_category(){
		...
		if($query->num_rows()>0){
			...
			$q1 = $this->db->where('id',$q_id)->update('db_expense_category', $info);
			if ($q1){
	...
	public function update_status($id,$status){
       if (set_status_of_table($id,$status,'db_expense_category')){
            echo "success";
        }
        else{
            echo "failed";
	...
	public function delete_categories_from_table($ids){
		...
		if($tot->num_rows() > 0){
			...
			exit();
				...
				$query1=$this->db->delete("db_expense_category");
		        if ($query1){
		            echo "success";
		        }
		        else{
