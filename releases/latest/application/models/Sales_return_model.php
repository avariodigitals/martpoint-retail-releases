...
class Sales_return_model extends CI_Model {
	...
	public function verify_save_and_update(){
	    ...
	    if($command=='save' || $command=='create'){//Create sales code unique if first time entry
		    ...
		    $sales_entry = array(
		    				...
		    				'return_code' 				=> get_init_code('sales_return'), 
	...
	public function update_sales($data){
		//Validate This sales already exist or not
		$this->db->trans_begin();
		$data = $this->xss_html_filter($data);
		$item_name = isset($data['item_name']) ? $data['item_name'] : '';
		$q_id = isset($data['q_id']) ? $data['q_id'] : '';
		$category_name = isset($data['category_name']) ? $data['category_name'] : '';
		$sales_price = isset($data['sales_price']) ? $data['sales_price'] : '';
		$hsn = isset($data['hsn']) ? $data['hsn'] : '';
		$alert_qty = isset($data['alert_qty']) ? $data['alert_qty'] : '';
		$unit_name = isset($data['unit_name']) ? $data['unit_name'] : '';
		$gst_percentage = isset($data['gst_percentage']) ? $data['gst_percentage'] : '';

		$this->db->where("upper(item_name)", strtoupper($item_name));
		$this->db->where("id !=", $q_id);
		if($this->db->get('db_sales')->num_rows()>0){
			return "This sales Name already Exist.";
		}
		else{
			$category_id = $this->db->select("id")->where("category_name", $category_name)->get("db_category")->row()->id;
			$info = array(
			...
			$this->db->where('id', $q_id);
			if ($this->db->update('db_sales', $info)){
				   $this->db->trans_commit();
			        return "success";
			}
			else{
