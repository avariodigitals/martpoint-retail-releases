...
class Currency_model extends CI_Model {
	...
	public function verify_and_save(){
		...
		$this->db->where("upper(currency_name)", strtoupper($currency_name));
		...
		if($query->num_rows()>0){
			...
			if ($this->db->insert('db_currency', $info)){
	...
	public function update_currency(){
		...
		if($query->num_rows()>0){
			...
			if ($this->db->where('id', $q_id)->update('db_currency', $info)){
	...
	public function update_status($id,$status){
       $this->db->where("id",$id);
	    ...
	    $query1=$this->db->update('db_currency');
        if ($query1){
            return "success";
        }
        return "failed";
