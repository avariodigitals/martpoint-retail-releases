...
class Templates_model extends CI_Model {
	...
	public function verify_and_save(){
		...
		$query = $this->db->get('db_smstemplates');
		if($query->num_rows()>0){
			...
			$info = array('template_name' => $template_name, 'content' => $content, 'status' => 1, 'store_id' => $store_id);
			if ($this->db->insert('db_smstemplates', $info)){
	...
	public function get_details($id,$data){
		//Validate This category already exist or not
		$store_id = get_current_store_id();
		$query=$this->db->query("select * from db_smstemplates where upper(id)=upper('$id') and store_id=".$store_id);
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
	...
	public function update_template(){
		...
		if($query->num_rows()>0){
			...
			$info = array('template_name' => $template_name, 'content' => $content);
			$this->db->where('id', $q_id);
			$this->db->where('store_id', $store_id);
			if ($this->db->update('db_smstemplates', $info)){
	...
	public function update_status($id,$status){
       if (set_status_of_table($id,$status,'db_smstemplates')){
            echo "success";
        }
        else{
            echo "failed";
	...
	public function delete_template($id){
		...
		$tot_count=$this->db->query("select count(*) as tot_count from db_smstemplates where id=$id and undelete_bit=1 and store_id=".$store_id)->row()->tot_count;
		if($tot_count>0){
			echo "Sorry! Can't Delete.\nThis Template Restricted!";exit();
		}
        $query1="delete from db_smstemplates where id=$id and store_id=".$$store_id;
