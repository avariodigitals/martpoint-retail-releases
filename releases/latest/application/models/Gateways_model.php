...
class Gateways_model extends CI_Model {
	...
	public function update_gateways(){
		$paypal_sandbox = $this->input->post('paypal_sandbox', TRUE);
		$paypal_email = $this->input->post('paypal_email', TRUE);
		$mojo_sandbox = $this->input->post('mojo_sandbox', TRUE);
		$mojo_api_key = $this->input->post('mojo_api_key', TRUE);
		$mojo_api_token = $this->input->post('mojo_api_token', TRUE);
		...
		$other_details = $this->input->post('other_details', TRUE);
	...
	public function save_payment(){
		...
		if($amount>0 && !empty($payment_type)){
			$bulk_payment = $amount;
			//Get Opening Balance
			$res2 = $this->db->where('id', $gateway_id)->get('db_gateways')->row();
			$gateway_opening_balance=$res2->opening_balance;
	    	$gateway_sales_due=$res2->sales_due;
