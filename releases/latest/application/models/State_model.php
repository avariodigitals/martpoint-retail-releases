...
class State_model extends CI_Model {
	...
	public function verify_and_save(){
		//Explicit POST extraction (security: never use extract on $_POST)
		$state = $this->input->post('state', TRUE);
		$country = $this->input->post('country', TRUE);
		$store_id_input = $this->input->post('store_id', TRUE);

		//Validate This state already exist or not
		$store_id=(store_module() && is_admin()) ? $store_id_input : get_current_store_id();
		$this->db->where('UPPER(state)', strtoupper($state));
		$this->db->where('UPPER(country)', strtoupper($country));
		$this->db->where('store_id', $store_id);
		$query = $this->db->get('db_states');
		if($query->num_rows()>0){
	...
	public function get_details($id){
		//Validate This state already exist or not
		$query = $this->db->where('id', $id)->get('db_states');
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
