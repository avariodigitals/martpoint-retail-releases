...
class Ninverify extends MY_Controller {
    ...
    public function check_bnpl_eligible(){
        ...
        if(empty($customer->email) && empty($customer->mobile)){
            echo json_encode(['eligible'=>false,'reason'=>'Customer phone number or email is required for PayPlan.']);
            return;
        }

        echo json_encode(['eligible'=>true,'reason'=>'Customer is eligible for PayPlan.']);
    }
