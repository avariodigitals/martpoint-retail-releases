					...
					if(result[0]=="success")
					{
						...
						if(print_done){
							if(command=='update'){
								...
								// Set WhatsApp share URL and show button
								...
								if(custMobile.indexOf('00') === 0) {
									custMobile = custMobile.substring(2);
								}
...
function hold_invoice_delete(invoice_id){
  swal({ title: "Are you sure?",icon: "warning",buttons: true,dangerMode: true,}).then((sure) => {
    if(!sure) return;
    // Require approval for deleting a hold invoice
