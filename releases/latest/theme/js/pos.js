...
$('#multiple-payments-modal').on('hidden.bs.modal', function(){
	// Always exit PayPlan mode when the payment modal closes
	bnplActive = false;
	$('#bnpl_section').hide();
...
$("#pay_all").on("click",function(){
	...
	if($('#pay_all').prop('disabled')){
		return;
	}
	if(isWalkInCustomer()){
		toastr["error"]("PayPlan is not available for walk-in customers. Please create a registered customer.");
		failed.currentTime = 0;
		failed.play();
		return;
	}

	function openPayPlanModal(customer){
		if(!customer || (!customer.mobile && !customer.email)){
			toastr["error"]("Customer must have a phone number or email to use PayPlan.");
			failed.currentTime = 0;
			failed.play();
			return;
		}
		// Activate PayPlan mode and open payment modal
		bnplActive = true;
		adjust_payments();
	...
	if(window.posCustomerDetails){
		openPayPlanModal(window.posCustomerDetails);
	} else {
		var base_url = $("#base_url").val();
		var cid = $("#customer_id").val();
		if(!cid){
			toastr["error"]("Please select a customer to use PayPlan.");
			failed.currentTime = 0;
			failed.play();
			return;
		}
		$.post(base_url + 'customers/get_customer_details', { customer_id: cid }, function(customer){
			...
			toastr["error"]("Unable to load customer details. Please try again.");
		});
	}
});
...
function processSaveSuccess(result, print, command, customer_id){
  var base_url = $("#base_url").val();
  // Always close the payment modal so the POS screen is usable again
  ...
  if(result[0]=="success"){
    ...
    if(command=='update'){
      ...
      var custMobile = (window.pos_customer_mobile || '').replace(/\D/g, '');
      if(custMobile.indexOf('00') === 0) { custMobile = custMobile.substring(2); }
      if(custMobile.indexOf('0') === 0 && custMobile.length === 11) { custMobile = '234' + custMobile.substring(1); }
      var waUrl = custMobile
        ? "https://wa.me/" + custMobile + "?text=" + encodeURIComponent(waText)
        : "https://api.whatsapp.com/send?text=" + encodeURIComponent(waText);
      if(print){
        // Open receipt in a modal (no new window). The receipt view auto-prints on load.
        $('#pos-receipt-iframe').attr('src', base_url + 'pos/print_invoice_pos/' + result[1]);
        $('#pos-receipt-whatsapp').attr('href', waUrl);
        $('#multiple-payments-modal').modal('hide');
...
function printPosReceipt(){
  var iframe = $('#pos-receipt-iframe')[0];
...
function doSave(print=false,pay_all=false){
	...
	// Append sales_note from payment modal if it exists
	var salesNote = $('#sales_note').val();
	if(salesNote){
		data.append('sales_note', salesNote);
		console.log('Appending sales_note:', salesNote);
	} else {
		console.log('No sales_note found in payment modal');
	}
	...
	$.ajax({
		...
		error: function(xhr){
			...
			$(".overlay").remove();
			$("#make_sale").attr('disabled',false);
			console.error("POS Save Error:", xhr.status, xhr.statusText);
			console.error("Response:", xhr.responseText);
			
			// Only offer offline queue if it's a genuine network error (0 or timeout)
			// For server errors (500), show the actual error message
...
$('#show_cash_modal').on("click",function (e) {
	...
	if($(".items_table tr").length==1){
    	...
    	$("#amount_1").focus();
    	$("#amount_1").parent().parent().removeClass('col-md-6').addClass('col-md-12');
    	// Default Pay (cash) to the current cashier's till account
    	var tillAccount = $("#pos_till_account_id").val() || '';
    	if(tillAccount){ $("#account_id_1").val(tillAccount); }
    	$('#multiple-payments-modal').modal('toggle');
    }
}); //hold_invoice end
            ...
            if(service_bit==0 && package_bit==0 && parseFloat(stock)<=0){
              if(typeof MPApproval !== 'undefined'){
                MPApproval.request('negative_stock_sale', {item_id: item_id, stock: stock}, function(approved, ctx){
                  ...
                  mpPosGetItemDetails(item_id, barcodeData);
                  $("#item_search").val('');
                });
                return false;
              } else {
