...
$("#item_search").autocomplete({
    source: function(data, cb){
        ...
        if($("#customer_id").val()==null){
          ...
          $("#customer_id").select2('open').next().find('input.select2-search__field').focus();
...
function delete_quotation(q_id)
{
  ...
  if(typeof swal === 'undefined'){
    ...
    doDeleteQuotation(q_id, base_url);
    return;
  }
  swal({
    title: "Delete Record?",
    text: "Do you want to delete this quotation record?",
    icon: "warning",
    buttons: true,
    dangerMode: true
  }).then(function(willDelete){
    if(willDelete) doDeleteQuotation(q_id, base_url);
  });
}
function doDeleteQuotation(q_id, base_url){
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
