    ...
    if(this_id=="save")  //Save start
    {
					//swal({ title: "Are you sure?",icon: "warning",buttons: true,dangerMode: true,}).then((sure) => {
					  if(confirm("Are you sure ?")) {//confirmation start
						...
						$.ajax({
						type: 'POST',
						url: base_url+'email_templates/newtemplate',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
	...
	else if(this_id=="update")  //Save start
    {
					...
					//swal({ title: "Are you sure?",icon: "warning",buttons: true,dangerMode: true,}).then((sure) => {
					  if(confirm("Are you sure ?")) {//confirmation start
						...
						$.ajax({
						type: 'POST',
						url: base_url+'email_templates/update_template',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
function delete_template(q_id)
{
	...
	if(typeof swal === 'undefined'){
		...
		doDeleteEmailTemplate(q_id, base_url);
		return;
	}
	swal({
		...
		if(willDelete) doDeleteEmailTemplate(q_id, base_url);
	});
}
function doDeleteEmailTemplate(q_id, base_url){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
