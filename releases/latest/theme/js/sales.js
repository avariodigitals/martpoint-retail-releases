...
$('#save,#update').on("click",function (e) {
  ...
  var this_id=this.id;
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    // WALK-IN CUSTOMER CANNOT HAVE CREDIT — must pay in full or create a customer profile
    if(this_id=='save' && isWalkInCustomer()){
      var total_amt = parseFloat($("#total_amt").text()) || 0;
      var paid_amt = parseFloat($("#amount").val()) || 0;
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
         ...
         // alert(result);return;
				result=result.split("<<<###>>>");
					...
					{
						// Show plain text errors as toastr instead of ugly alert
						var errMsg = result[0] || "";
						if(errMsg.length > 0){
							toastr["error"](errMsg);
    ...
    source: function(data, cb){
     ...
     response:function(e,ui){
        ...
        select: function (e, ui) {
            ...
            if(typeof ui.content!='undefined' && ui.content[0].expired){
              is_expired = true;
            }
            else if(typeof ui.item!='undefined' && ui.item.expired){
              is_expired = true;
            }
            if(is_expired){
            ...
            if(service_bit==1){
              return_row_with_data(item_id, barcodeData);
            }
            else {
...
function return_row_with_data(item_id, barcodeData){
  ...
  var price_type=$("#price_type").val() || 'wholesale';
  var postData = {customer_id:customer_id,warehouse_id:warehouse_id,price_type:price_type};
  if(barcodeData && barcodeData.barcode){
    postData.barcode = barcodeData.barcode;
    postData.batch_lot = barcodeData.batch_lot || '';
  }
  $.post(base_url+"sales/return_row_with_data/"+rowcount+"/"+item_id,postData,function(result){
        //alert(result);
        // Handle JSON error responses gracefully
        try {
